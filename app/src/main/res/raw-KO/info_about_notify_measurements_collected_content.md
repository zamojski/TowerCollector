...
Tower Collector은 *intents*를 브로드캐스트하여 장치에 설치된 다른 앱에 전송할 수 있습니다. 이러한 알림을 수신하고 사용하는
방법에 대한 자세한 내용은 해당 앱의 도움말을 확인하세요.

### 데이터 수집 이벤트

설정된 경우 새로운 데이터가 측정 될 때마다 intent를 브로드캐스트합니다. 알림의 내용은 시간 및 GPS와 같은 JSON 구조의
데이터입니다. 필드 이름과 값은 CSV를 내보낼 때 내용과 동일한 방식으로 맵핑됩니다.
