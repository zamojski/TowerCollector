...
Tower Collector可以通过广播 *intents*来通知系统上安装的其他应用。 有关如何监听和使用此类通知的更多信息，请检查应用的帮助。

### 测量收集事件

如果在"设置"中配置，则每次收集新的测量集时，收集器都可以发送意图。 通知内容将是一个JSON结构，其中包含时间和GPS等常见内容。 字段名称和值的映射方式与导出的CSV文件中的映射方式相同。
